...
`check-secrets.js` падает (код 1), если в отслеживаемых файлах найден токен бота, приватный ключ, пароль/секрет в коде, JWT, строка подключения к БД или отслеживаемый `.env`. Секреты живут ТОЛЬКО в `.env` на VPS (права 600, в .gitignore). Проверять оба скрипта перед каждым пушем.

## Технические правила

- Vanilla JS: IIFE, `'use strict'`, 2 пробела, единый неймспейс `window.CS`; никаких модулей, сборки, CDN. **Исключение 1:** официальный `https://telegram.org/js/telegram-web-app.js` — подключён в index.html; вне Telegram/офлайн он не загружается, а весь код проверяет `window.Telegram` перед использованием. **Исключение 2:** `wss://…/ws` — собственный WebSocket-сервер дуэлей (SPEC §28, VPS); соединение открывается `CS.Net.join` только при входе в режим дуэли; офлайн/обычная игра и file:// без дуэли сеть не трогают вовсе. Библиотек с CDN больше нет — нативный WebSocket.
